...
def teacher_dashboard():
    ...
    with tab1:
        type1 = "primary" if st.session_state.curr_teacher_tab == "take_attendance" else "secondary"
        if st.button(label = "Take Attendance",type=type1,width="stretch",icon=":material/ar_on_you:"):
            st.session_state.curr_teacher_tab = "take_attendance"
            st.rerun()
    with tab2:
        type2 = "primary" if st.session_state.curr_teacher_tab == "manage_subjects" else "secondary"
        if st.button(label ="Manage Subjects",type = type2,width="stretch",icon=":material/book_ribbon:"):
            st.session_state.curr_teacher_tab = "manage_subjects"
            st.rerun()
    with tab3:
        type3 = "primary" if st.session_state.curr_teacher_tab == "attendance_records" else "secondary"
        if st.button(label = "Attendance Records",type=type3,width="stretch",icon=":material/cards_stack:"):
            st.session_state.curr_teacher_tab = "attendance_records"
            st.rerun()
    st.divider()
    if st.session_state.curr_teacher_tab == "take_attendance":
        teacher_tab_take_attendance()
    if st.session_state.curr_teacher_tab == "manage_subjects":
        teacher_tab_manage_subjects()
    if st.session_state.curr_teacher_tab == "attendance_records":
        ...
        if st.button("Run Face Analysis",width = "stretch",type = "secondary",icon=":material/analytics:",disabled=not has_photos):
            with st.spinner("Deep Scanning Classroom Photos..."):
                ...
                if not enrolled_students:
                    st.warning("No Student Enroll in this Course")
                else:
...
def teacher_tab_manage_subjects():
    ...
    with col1:
        st.header("subjects",width="stretch")
    with col2:
...
def teacher_screen_login():
    ...
    teacher_username = st.text_input("Enter Username",placeholder="nitin",key="teacher_username")
    teacher_password = st.text_input("Enter Password",type="password",placeholder="Enter Password",key="teacher_password")
    st.divider()
...
def register_teacher(teacher_username,teacher_name,teacher_password,check_password):
    ...
    try:
        create_teacher(teacher_username,teacher_password,check_password)
...
def teacher_screen_register():
    ...
    check_password = st.text_input("confirm your password",type="password",placeholder="confirm your password")
    st.divider()
    btnc1 , btnc2 = st.columns(2,gap="large")
    with btnc1:
